...
* `B[j-1] <= A[i] && A[i-1] <= B[j]`,  B的前一半元素小于A的分隔符, A的前一半元素小于B的分隔符
...
如果约定不能更改当前字符串, 最好用C++做, 不然操作字符串太复杂了, 没必要出错。
...
乍一看如果需要进位的话, 可能需要拷贝整个数组. 实际上并不需要, 我们知道只有当数字是999...999的时候，才会使得数字的长度+1变为1000...000。
